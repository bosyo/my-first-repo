...
projectApi.addProject = async function (projectName) {
  const response = await this.call('add.project', {projectName});
  return response
};

projectApi.deleteProject = async function (projectId) {
  const response = await this.call('delete.project', {projectId});
  return response
}
