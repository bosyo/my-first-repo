...
class Register extends Component {
  ...
  _renderLogo = () => {
    return (
      <View style={[bs.f_width('100%'), bs.content_center, bs.item_center, bs.f_mb(10)]}>
        <Image source={logo} style={[bs.f_width(250), bs.f_height(100), {resizeMode: 'contain'}]}/>
      </View>
    )
  };
  _renderForm = () => {
    return (
      <View style={[bs.f_width('100%'), bs.f_ph(20)]}>
        {this._renderInput('flag', 'COUNTRY:', this.state.country, (val) => {
          this.setState({country: val});
        })}
        {this._renderInput('envelope-o', 'EMAIL:', this.state.email, (val) => {
          this.setState({email: val});
        })}
        {this._renderInput('key', 'PASSWORD:', this.state.password, (val) => {
          this.setState({password: val});
        }, true)}
        {this._renderInput('key', 'CONFIRM PASSWORD:', this.state.confirmPassword, (val) => {
  ...
  _renderRegisterButton = () => {
    return (
      <View style={[bs.flex_row, bs.f_mb(20), bs.f_height(35)]}>
        ...
        >
          <Text style={[bs.f_color('white'), {alignSelf: 'center'}, bs.f_fontSize(12)]}>SIGN UP</Text>
        </TouchableOpacity>
      </View>
    )
  }
  _renderLinks = () => {
    return (
      <View style={[bs.f_width('100%'), bs.f_ph(20), bs.item_center, bs.f_mb(20)]}>
